...
public class MineralOrderDetector extends DogeCVDetector {
    ...
    public Mat process(Mat input) {
        ...
        if(choosenWhiteRect.get(0) != null && choosenWhiteRect.get(1) != null  && chosenYellowRect != null){
            ...
            if(leftCount == 0){
                currentOrder = MineralOrderDetector.GoldLocation.LEFT;
            }

            if(leftCount == 1){
                currentOrder = MineralOrderDetector.GoldLocation.CENTER;
            }

            if(leftCount >= 2){
                currentOrder = MineralOrderDetector.GoldLocation.RIGHT;
            }
            isFound = true;
            lastOrder = currentOrder;

        }else{
            currentOrder = MineralOrderDetector.GoldLocation.UNKNOWN;
            isFound = false;
        }
